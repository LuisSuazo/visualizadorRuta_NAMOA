...
public class Escritura {
		...
		try {
			...
			for (Calle aux : Calles) {
				pw.print( "\t\t\t"+"<Placemark>\n"+
						...
						"\t\t\t\t"+"<SimpleField name=\"PobExp1000\">"+aux.getPobExpuesta1000()+"</SimpleField>\n" +
						"\t\t\t\t"+"<SimpleField name=\"Riesgo800\">"+aux.getRiesgo800()+"</SimpleField>\n" +
						"\t\t\t\t"+"<SimpleField name=\"Riesgo1000\">"+aux.getRiesgo1000()+"</SimpleField>\n" +
	...
	public void NodosRiesgoKML(List<Circulo> puntosRiesgo) {
		...
		try {
			...
			fichero = new FileWriter("KML_Output/"+"NodosRiesgo"+".kml"); /// Escribe el KML con el nombre de la comuna
			...
			puntosRiesgo.forEach(it -> {
				...
				pw.println("<Placemark>\r\n" + 
						...
						"			<coordinates>"+
						it.getCentro().getY()+","+it.getCentro().getX()+",0"+
						"</coordinates>\r\n" + 
						"		</Point>\r\n" + 
						"	</Placemark>");
				pw.println("<Folder>\r\n" + 
	...
	public void NodosRiesgoCSV(List<Nodo> puntosRiesgo) {
		...
		try {
			...
			fichero = new FileWriter("KML_Output/"+"NodosRiesgo"+".csv"); /// Escribe el KML con el nombre de la comuna
			final PrintWriter pw = new PrintWriter(fichero);
			String delimidator=",";
			pw.println("Num"+delimidator+"Demanda"+delimidator+"Longitude"+delimidator+"Latitude"+delimidator+"Radius"+delimidator+"Style"+delimidator+"Description");
			puntosRiesgo.forEach(it -> {
				pw.println(it.getId()+delimidator+it.getDemanda()+delimidator+it.getY()+delimidator+it.getX()+"1000"+"101"+"algo");
			});
			pw.close();
			fichero.close();
		}catch(IOException e) { e.printStackTrace(); }
		...
		try {
			...
			for (Entry<Integer, Nodo> aux : Puntos.entrySet()) {
				pw.println("<Placemark>\r\n" + 
						...
						"			<gx:drawOrder>1</gx:drawOrder>\r\n" + 
						"			<coordinates>"+
						aux.getValue().getY()+","+aux.getValue().getX()+",0"+
						"</coordinates>\r\n" + 
						"		</Point>\r\n" + 
						"	</Placemark>");
				pw.flush();
		...
		try {
			...
			for (Entry<Integer,ArrayList<Nodo>> it : Ruta.entrySet()) {
				...
				for(int i=1;i<it.getValue().size();++i) {
					String Consultar=String.valueOf(it.getValue().get(i-1).getId()+"-"+it.getValue().get(i).getId());
					...
					pw.println("<Placemark>\r\n" + 
							...
							"\t\t\t\t\t"+"<SimpleData name=\"PobExp800\">"+aux.getPobExpuesta800()+"</SimpleData>\n" +
							"\t\t\t\t\t"+"<SimpleData name=\"PobExp1000\">"+aux.getPobExpuesta1000()+"</SimpleData>\n" +
	...
	private void EscribirStyleKML(PrintWriter pw,int numerito){
		...
		for(int i=0;i<numerito;i++){
			...
			pw.println(	"<StyleMap id=\"failed"+i+"\">"+
					"<Pair>"+
					"	<key>normal</key>"+
					"	<styleUrl>#failed1"+i+"</styleUrl>"+
					" </Pair>"+
					" <Pair>"+
					"	<key>highlight</key>"+
					"	<styleUrl>#failed0"+i+"</styleUrl>"+
					" </Pair>"+
					"</StyleMap>"+
					"<Style id=\"failed0"+i+"\">"+
					"<LineStyle>"+
